...
use gimli::{
    ...
    FrameDescriptionEntry, LittleEndian, Reader, Register, SectionBaseAddresses, UnwindSection,
    X86_64,
};
...
impl EhInstrContext {
    pub fn print<S, R>(&mut self, section: &S, instr: CallFrameInstruction<R::Offset>)
    where
        S: UnwindSection<R>,
        R: Reader,
    {
        ...
        match instr {
            ...
            DefCfaExpression { expression } => {
                println!(
                    ...
                    expression.get(section).unwrap().0.to_slice().unwrap()
                );
            }
            Undefined { register } => {
                println!(
            ...
            // FIXME: Unfortunately, CallFrameInstruction is now #[non_exhaustive].
            // This means we are forced to include a catch-all. Let's hope someone remembers to
            // update this. I'll attempt to remove #[non_exhaustive] upstream, so we can ditch this
            // if it gets merged: https://github.com/gimli-rs/gimli/pull/764
            _ => todo!(),
        }
    }
}
